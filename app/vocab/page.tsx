              ...
              >
                ...
                <div className="flex gap-1">
                  <div className="relative">
                    <button onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === item.id ? null : item.id); }} className="p-3 text-gray-300 active:bg-gray-100 rounded-full transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z" /></svg>
                    </button>
                    {activeMenuId === item.id && (
                      <div className="absolute right-0 top-full mt-2 w-32 bg-white rounded-2xl shadow-xl border border-gray-100 z-10 overflow-hidden py-1">
                        <button onClick={(e) => { e.stopPropagation(); handleEdit(item); setActiveMenuId(null); }} className="w-full px-4 py-3 text-left text-sm active:bg-[#00C9A7]/10 text-gray-700 transition-colors">Sunting</button>
                        <button onClick={(e) => { e.stopPropagation(); setDeleteConfirmationId(item.id); setActiveMenuId(null); }} className="w-full px-4 py-3 text-left text-sm active:bg-red-50 text-red-600 transition-colors">Padam</button>
                      </div>
                    )}
                  </div>
                </div>
